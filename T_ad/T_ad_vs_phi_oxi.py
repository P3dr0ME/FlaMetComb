...
ideal_species = [species_dict[S] for S in ("CH4", "O2", "CO2", "H2O")]
...
M_O2 = 32e-3 # kg/mol
f_s = 1*M_CH4/(2*M_O2) # Dosado estequiométrico
...
for i in range(len(phi_list)):
    ...
    gas_ideal.set_equivalence_ratio(phi, "CH4", "O2")
    ...
    gas_ideal_aux.set_equivalence_ratio(phi, "CH4", "O2")
    gas_ideal_aux.TP = (T_ad_ideal[i]+T_0)/2, p
    cp_ave[i] = gas_ideal_aux.cp # lista de cp de productos como base de datos para calcular T_ad analítica
    if 0<phi<=1: # pobre
...
plt.savefig("./plots/T_ad/T_ad_vs_phi_oxi.svg")
plt.show()
    # muestra el gráfico
