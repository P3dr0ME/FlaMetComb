...
for phi in phi_list:
    ...
    cp_m1[phi] = gas_ideal.cp

    if 0<phi<=1: # pobre
        T_ad_m1[phi] = T_r + ( phi*f_s*LHV ) / ( (1+phi*f_s)*cp_m1[phi] )
    elif phi>1: # rica
        T_ad_m1[phi] = T_r + ( f_s*LHV ) / ( (1+phi*f_s)*cp_m1[phi] )

    q0_p_m1[phi] = (T_ad_m1[phi]-T_r)*cp_m1[phi] / 1e6 # MJ/kg
...
Tabla_q0_p.index.name = "\u03c6" # Cabecero columna primera

# print(Tabla_q0_p.to_csv())
print(Tabla_q0_p)
...
Tabla_c_p.index.name = "\u03c6" # Cabecero columna primera

# print(Tabla_c_p.to_csv())
print(Tabla_c_p)
...
plt.plot(phi_list, q0_p_real.values(), '.-', label="Cantera")
plt.plot(phi_list, q0_p_ideal.values(), '.-', label="Cantera (ideal)")
...
plt.savefig(f"./plots/T_ad/q0_p_vs_phi_{type}.svg")
plt.show()
