...
gas_com = ct.Solution(thermo="ideal-gas",
                               ...
                               kinetics='gas')
# gas_com() muestra todos la info sobre el objeto ct.Solution.
...
gas = ct.Solution(thermo="ideal-gas",
                                 ...
                                 transport_model='mixture-averaged',
                                 kinetics='gas')
# print(f"Transport model: {gas.transport_model}")
...
for phi in range(len(phi_list)):
    #print("%"*80)
    #print(f"Equivalence ratio {phi_list[phi]}")
    ...
    gas_com.set_equivalence_ratio(phi_list[phi], "CH4", "O2:1, N2:3.76")
        # CH4 no lleva X porque no hay más especies en fuel (X=1).

    gas.set_equivalence_ratio(phi_list[phi], "CH4", "O2:1, N2:3.76")
    ...
    #print(f"T_ad_complete = {T_ad_complete[phi]}")
    #print(f"T_ad_incomplete = {T_ad_incomplete[phi]}")
...
plt.savefig("plots/T_ad_vs_phi_air.svg")
plt.show()
    # muestra el gráfico
